...
if ( ! function_exists('savoya_google_fonts') ) :
function savoya_google_fonts() {

	if ( savoya_theme_option( 'default_theme_fonts', 'default_fonts' ) == 'google_fonts' ) :

		$main_font 					= savoya_theme_option( 'main_font', 'embed-jaden');
		$main_font_variants 		= savoya_theme_option( 'main_font_variants', array('400'));

		$secondary_font 			= savoya_theme_option( 'secondary_font', 'Roboto');		
		$secondary_font_variants 	= savoya_theme_option( 'secondary_font_variants', array('400'));

		$google_fonts_subsets 		= savoya_theme_option( 'google_fonts_subsets', '');
		...
		if ( !empty($font_family) ):
			...
			wp_enqueue_style( 'savoya_google_fonts', $fonts_url, array(), null );

		endif;

	endif;
}            
add_action('wp_head', 'savoya_google_fonts', 0);
endif;